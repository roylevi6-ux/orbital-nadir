            ...
            <main className="p-8 space-y-8 animate-in fade-in">
                ...
                <div className="flex justify-between items-end">
                    <div>
                        ...
                        <p className="text-slate-400">Track your assets across all currencies.</p>
                    </div>
                ...
                <div className="bg-gradient-to-br from-violet-900/50 to-slate-900 border border-white/10 rounded-3xl p-8 relative overflow-hidden">
                    ...
                    <p className="text-slate-400 mt-2 text-sm max-w-md relative z-10">
                    ...
                    {Object.entries(byType).map(([type, typeAccounts]) => (
                        <div key={type}>
                            <h3 className="text-xl font-bold text-white capitalize mb-4 flex items-center gap-2">
                                {type} <span className="text-xs bg-slate-800 text-slate-400 px-2 py-1 rounded-full">{typeAccounts.length}</span>
                            </h3>
                ...
                {accounts.length === 0 && (
                    <div className="text-center py-12 border-2 border-dashed border-white/5 rounded-3xl">
                        <p className="text-slate-500 mb-4">No accounts tracked yet.</p>
                        <NewAccountModal onComplete={loadData} />
                    </div>
                )}
            </main>
